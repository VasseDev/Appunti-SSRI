...
1.  $$
    ...
    Forma trigonometrica:
    $$
    \rho(\cos\theta+i\sin\theta)=z(\cos(\frac{\pi}{6})+i\sin(\frac{\pi}{6}))
    $$
2.  $$
    z=i
    $$
...
> Da qui si ricava la formila delle potenze che è il caso $$z_1=z_2=\dots=z_n=z=\rho(\cos\theta+i\sin\theta)$$ > $$z^n=\rho^n(\cos(n\theta)+i\sin(n\theta))$$
